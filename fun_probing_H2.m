function H2_sol = fun_probing_H2(C1, C2, ry, ru, H1, w1_ax, dts)
...
for j = 1: numel(w1_ax)
    for i = 1: numel(w1_ax)
	  ...
	  continue
        ...
        [~, R, freq] = fun_residual2(C1, C2, H1w1, H1w2, z1, ry, ru, w1, w2, dts, t) ; % Compute the residual for Point 1
        [~, index] = min(abs(freq - (w1+w2)/(2*pi) )) ; % Identify the correct index (freqeuncy bin)
        delta_1 =  R(index);


        [~, R, freq] = fun_residual2(C1, C2, H1w1, H1w2, z2, ry, ru, w1, w2, dts, t) ; % Compute the residual for Point 2
        [~, index] = min(abs(freq - (w1+w2)/(2*pi) )) ; % Identify the correct index (freqeuncy bin)
        delta_0 =  R(index);
