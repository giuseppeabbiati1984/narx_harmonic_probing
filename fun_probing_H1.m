function H1_sol = fun_probing_H1(C1, ry, ru, w1_ax, dts)
...
for i=1:1:numel(w1_ax)
    ...
    [~, R, freq] = fun_residual1(C1, z1,  ry, ru, w1, dts, t) ; % Compute the residual for Point 1. Line 20 (pg. 6)
    [~, index] = min(abs(freq - (w1)/(2*pi) )) ; % Identify the correct index for the optimization
    delta_1 =  R(index);

    [~, R, freq] = fun_residual1(C1, z2,  ry, ru, w1, dts, t) ; % Compute the residual for Point 2. Line 19 (pg. 6)
    [~, index] = min(abs(freq - (w1)/(2*pi) )) ; % Identify the correct index for the optimization
    delta_0 =  R(index);
