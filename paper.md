...
An important nonlinear system identification approach relies on the Volterra and Wiener functional series [@billings_identification_1980], [@schetzen_1980_volterra]. The theoretical basis for these methods was established by the pioneering works of Volterra, who introduced the theory of analytical functions [@volterra_sopra_1887] and later Frechet, who used it to represent continuous nonlinear systems [@frechet_sur_1910]. Early efforts to identify nonlinear models, notably the works of Wiener, Bose, and Barrett [@wiener_nonlinear_1958], [@bose_theory_1956], [@barrett_hermite_1964], revolved around a direct estimation of the Volterra kernels (or of the GFRFs, which are the frequency-domain counterparts of Volterra kernels), from measured data. However, estimating GFRFs directly from raw data requires a large number of identified parameters, namely kernel points, to adequately represent the Volterra kernels, and consequently demands large amounts of input–output measurement data.
...
The present software computes the GFRFs based on input-output data, $u(t)$ and $y(t)$, provided by the user. The technical details of this computation are presented in the theoretical basis for this software presented by [@stamenov_numerical_2025].
