function NARX = fun_poly_NARX(Xf, Xz, Yf, ord, varargin)
...
% lag samples
NARX.lagNumF = size(Xf, 2) ;  % response signal
NARX.lagNumZ = size(Xz, 2) ;  % loading signal
...
string_fun_p = ['fun_p_nf' num2str(NARX.lagNumF), 'nz' , num2str(NARX.lagNumZ), '_ord', num2str(ord), '(x)'] ;
...
string_fun_J = strcat('fun_J_nf', num2str(NARX.lagNumF), 'nz' , num2str(NARX.lagNumZ), '_ord', num2str(ord), "(a_ls1_ext.')") ;
NARX.J(:) =  eval(string_fun_J)  ; % reconstruction of sparse Jacobian

if ord == "2" || ord == "3" || ord =="2O" || ord =="23O"
string_fun_H = strcat('fun_H_nf', num2str(NARX.lagNumF), 'nz' , num2str(NARX.lagNumZ), '_ord', num2str(ord), "(a_ls1_ext.')") ;
NARX.H(:, :) = eval(string_fun_H) ; % reconstruction of sparse Hessian
end

if ord == "3" || ord =="23O"
string_fun_T = strcat('fun_T_nf', num2str(NARX.lagNumF), 'nz' , num2str(NARX.lagNumZ), '_ord', num2str(ord), "(a_ls1_ext.')") ;
